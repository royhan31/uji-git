    ...
    public function store(Request $request){
      ...
      $loker = Loker::create([
        ...
        if(count($tokens) > 0){
          ...
          $notificationBuilder = new PayloadNotificationBuilder('Loker baru telah ditambahkan');
          $notificationBuilder->setBody('Loker '.$loker->name.' telah ditambahkan oleh Perusahaan '.$loker->company->company)
                      ->setSound('default');

          $dataBuilder = new PayloadDataBuilder();
          $dataBuilder->addData(['loker_id' => $loker->id]);
